...
`@docx4j/generated-objects-ts`: Office Open XML as typed JavaScript. `modules/` holds 96 Jsonix mappings
...
`textOf`, `runItemsOf`, content controls, rows and cells, inline pictures (CR-003), traversal; imports the helpers, never the reverse). The runtime is
