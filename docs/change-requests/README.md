...
| [CR-005](CR-005-module-registry.md) | A bundler-friendly way to load the mapping modules: a generated static registry in place of `getContext()`'s computed `import()`, and a `modules` option for a caller that wants a smaller context; requested by the editor (ED-002 section 10.3), which had to configure Vite to find the modules | Proposed 2026-09-24 |
