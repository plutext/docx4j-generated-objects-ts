...
| [CR-005](CR-005-module-registry.md) | A bundler-friendly way to load the mapping modules: a static registry of literal imports instead of one computed `import()`, and a `modules` option on `getContext` for a smaller context | Proposed 2026-09-24 (reviewed; the registry belongs in `src/`) |
